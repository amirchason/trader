'use strict';
// XRP Low-ATR regime + RSI7 — we found h=[6,9,12,18] works, add ATR filter
...
const XRP_HIGH=[6,12]; // tight hours: 71.2% WR
...
console.log('=== XRP Low-ATR + Confirm Signals ===\n');

console.log('--- XRP Low-ATR regime ---');
for(const pctT of [0.25,0.33,0.40,0.50]){
  for(const[bb,bLabel] of [[bb25_22,'BB(25,2.2)'],[bb20_22,'BB(20,2.2)'],[bb15_22,'BB(15,2.2)']]) {
    for(const hrs of [XRP_GOOD, new Set(XRP_HIGH)]) {
      for(const minS of [1,2]) {
        ...
        const hLabel=hrs.size===4?'h[6,9,12,18]':'h[6,12]';
        rpt(`XRP LowATR${(pctT*100).toFixed(0)}%+${bLabel}+${hLabel}+s>=${minS}`,sigs);
      }
    }
  }
}
...
console.log('\n--- XRP RSI7 + good hours ---');
for(const[bb,bLabel] of [[bb25_22,'BB(25,2.2)'],[bb20_22,'BB(20,2.2)']]) {
  for(const rsiT of [55,60,65]){
    for(const minS of [1,2]){
      ...
      rpt(`XRP RSI7>${rsiT}+${bLabel}+h[6,9,12,18]+s>=${minS}`,sigs);
    }
  }
}
...
for(const mfiT of [65,70,75,80]){
  for(const[bb,bLabel] of [[bb25_22,'BB(25,2.2)'],[bb20_22,'BB(20,2.2)']]) {
    ...
    rpt(`XRP MFI>${mfiT}+${bLabel}+h[6,9,12,18]+s>=1`,sigs);
  }
}
...
for(const rsiT of [55,60,65]){
  for(const[bb,bLabel] of [[bb25_22,'BB(25,2.2)'],[bb20_22,'BB(20,2.2)']]) {
    for(const minS of [1,2]){
      ...
      rpt(`XRP RSI14>${rsiT}+${bLabel}+h[6,9,12,18]+s>=${minS}`,sigs);
    }
  }
}
