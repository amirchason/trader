'use strict';
// XRP combo: LowATR + MFI + RSI7 — combining best XRP signals
const Database = require('better-sqlite3');
const path = require('path');
const db = new Database(path.join(__dirname, '../../trader.db'));
const raw5m = db.prepare('SELECT open_time,open,high,low,close,volume FROM candles WHERE symbol=? AND timeframe=? ORDER BY open_time ASC').all('XRP','5m');
...
const XRP_GOOD=new Set([6,9,12,18]);
...
function calcMFI(can,p){const m=new Array(can.length).fill(null);for(let i=p;i<can.length;i++){let pos=0,neg=0;for(let j=i-p+1;j<=i;j++){const tp=(can[j].high+can[j].low+can[j].close)/3,ptp=(can[j-1].high+can[j-1].low+can[j-1].close)/3,rf=tp*can[j].volume;if(tp>ptp)pos+=rf;else neg+=rf;}m[i]=neg===0?100:100-100/(1+pos/neg);}return m;}
...
const bb25_22=calcBB(closes,25,2.2),bb20_22=calcBB(closes,20,2.2),bb15_22=calcBB(closes,15,2.2);
...
console.log('=== XRP Combo Signals ===\n');

console.log('--- LowATR + MFI combo ---');
for(const pctT of [0.25,0.33,0.40]){
  for(const mfiT of [65,70,75]){
    const sigs=[];
    for(let i=100;i<candles.length-1;i++){
      if(!bb25_22[i]||mfi10[i]===null)continue;
      if(!XRP_GOOD.has(new Date(candles[i].open_time).getUTCHours()))continue;
      const p=atrPct(i);if(p===null||p>pctT)continue;
      const{streak,dir}=getStreak(i);if(streak<1)continue;
      if(dir===1&&closes[i]>=bb25_22[i].upper&&mfi10[i]>=mfiT)sigs.push({win:candles[i+1].close<candles[i+1].open});
      if(dir===-1&&closes[i]<=bb25_22[i].lower&&mfi10[i]<=(100-mfiT))sigs.push({win:candles[i+1].close>candles[i+1].open});
    }
    rpt(`XRP LowATR${(pctT*100).toFixed(0)}%+MFI>${mfiT}+BB(25,2.2)+GoodH+s>=1`,sigs);
  }
}
...
for(const pctT of [0.25,0.33,0.40]){
  for(const rsiT of [55,60,65]){
    for(const[bb,bLabel] of [[bb25_22,'BB(25,2.2)'],[bb15_22,'BB(15,2.2)']]) {
      const sigs=[];
      for(let i=100;i<candles.length-1;i++){
      ...
      rpt(`XRP LowATR${(pctT*100).toFixed(0)}%+RSI7>${rsiT}+${bLabel}+GoodH+s>=1`,sigs);
    }
  }
}

console.log('\n--- XRP BB(15,2.2) standalone (tight band) ---');
for(const minS of [1,2,3]) {
  for(const hrs of [[...XRP_GOOD],[6,12],[6,9,12]]) {
    ...
    rpt(`XRP BB(15,2.2)+h=[${hrs}]+s>=${minS}`,sigs);
  }
}
